...
<div class="max-w-2xl space-y-5">
    ...
    <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
        <div class="flex items-center gap-4 px-6 py-5 border-b border-slate-100 bg-slate-50/50">
            ...
            <div>
                ...
                <span class="inline-flex items-center gap-1.5 text-xs font-medium text-violet-600 bg-violet-50 border border-violet-100 px-2.5 py-1 rounded-full mt-1">
                    <i class="fa-solid fa-shield-halved text-xs"></i> Administrador
                </span>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.account.update') }}" class="p-6 space-y-4"
    ...
    <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.account.password') }}" class="p-6 space-y-4"
              onsubmit="this.querySelector('button[type=submit]').disabled=true">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Confirmar nueva contraseña *</label>
