...
@section('title', 'Conócenos – Novitecnología Cia. Ltda.')
...
<section class="py-24 px-6" style="background:linear-gradient(135deg,#f8fafc 0%,#eff6ff 100%);">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white border border-slate-100 rounded-2xl p-6 hover:border-blue-200 hover:shadow-md transition-all reveal" style="transition-delay:{{ $i * 0.08 }}s">
        ...
        <div class="relative reveal">
            ...
            <div class="flex justify-center gap-2 mt-5">
                ...
                <button onclick="goToSlide({{ $dot }})"
                        id="dot-{{ $dot }}"
                        class="w-2 h-2 rounded-full transition-all {{ $dot === 0 ? 'bg-blue-600 w-6' : 'bg-slate-300' }}">
                </button>
                @endforeach
            </div>
        </div>
...
{{-- SUCURSALES --}}
<section class="py-24 px-6" style="background:linear-gradient(135deg,#eff6ff 0%,#f8fafc 100%);">
...
function updateSlider() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        dot.classList.remove('bg-blue-600', 'w-6', 'bg-slate-300');
        if (i === current) {
